; -------------------------------------------------------------------------
; Task 1D: Measurement of Analog Voltage (ADC)
; -------------------------------------------------------------------------

; --- Memory Mapped Registers ---
RCC_AHB1ENR  EQU 0x40023830     ; Clocks for GPIO
RCC_APB1ENR  EQU 0x40023840     ; Clocks for I2C1
RCC_APB2ENR  EQU 0x40023844     ; Clocks for ADC1
...
ADC1_SQR3    EQU 0x40012034     ; ADC1 Sequence Register 3
ADC1_DR      EQU 0x4001204C     ; ADC1 Data Register

OLED_ADDR    EQU 0x78           ; 0x3C << 1
...
start PROC
    ...
    LDR R0, =RCC_AHB1ENR
    LDR R1, [R0]
    ORR R1, R1, #0x03           ; GPIOA (Bit 0) and GPIOB (Bit 1)
    STR R1, [R0]
    ...
    LDR R0, =RCC_APB2ENR
    LDR R1, [R0]
    ORR R1, R1, #(1 << 8)       ; ADC1 Enable (Bit 8)
    STR R1, [R0]

    ; 2. Configure PA1 as Analog Mode (Bits 2:3 = 11)
    LDR R0, =GPIOA_MODER
    LDR R1, [R0]
    ORR R1, R1, #(0x3 << 2)     
    STR R1, [R0]
    ...
    ; 4. Configure ADC1 (Turn ON and set Channel 1)
    LDR R0, =ADC1_CR2
    MOV R1, #1                  ; Set ADON bit (Bit 0)
    STR R1, [R0]
    LDR R0, =ADC1_SQR3
    MOV R1, #1                  ; Set 1st conversion to Channel 1 (PA1)
    STR R1, [R0]
...
init_loop
    ...
    ; 6. Print "VOLTMETER" on Row 1
    MOV R1, #0                  ; Page 0
    MOV R2, #0                  ; Column 0
    BL OLED_SetCursor
    LDR R4, =String_Volt_Title
    BL OLED_PrintString

    ; 7. Main ADC Polling Loop
main_loop
    ; Start ADC Conversion (Set SWSTART bit 30)
    LDR R0, =ADC1_CR2
    LDR R1, [R0]
    ORR R1, R1, #(1 << 30)      
    STR R1, [R0]
...
wait_adc
    ; Wait for EOC (End of Conversion bit 1)
    LDR R0, =ADC1_SR
    LDR R1, [R0]
    TST R1, #(1 << 1)
    BEQ wait_adc

    ; Read ADC Data (0 to 4095)
    LDR R0, =ADC1_DR
    LDR R1, [R0]
    ...
    LDR R2, =3300
    MUL R1, R1, R2
    LSR R1, R1, #12             ; R1 now contains Millivolts! (e.g., 1540)

    ; Extract Volts Digit
    MOV R2, #1000
    UDIV R3, R1, R2             ; R3 = Volts
    MLS R1, R3, R2, R1          ; R1 = Remainder

    ; Extract Tenths Digit
    MOV R2, #100
    UDIV R4, R1, R2             ; R4 = Tenths
    ...
    MOV R8, R3                  ; Volts digit
    BL OLED_PrintChar
    
    MOV R8, #0x2E               ; Decimal point '.'
    BL OLED_PrintChar
    
    MOV R8, R4                  ; Tenths digit
    BL OLED_PrintChar

    LDR R4, =String_V_Unit      ; " V   "
    BL OLED_PrintString
...
String_Volt_Title DCB "VOLTMETER      ", 0
String_V_Unit     DCB " V   ", 0
    ALIGN
