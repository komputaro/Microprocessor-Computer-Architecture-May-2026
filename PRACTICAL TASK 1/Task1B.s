...
start PROC
    ...
    ; -------------------------------------------------------------
    ; 2. Configure PB8 (SCL) & PB9 (SDA) to Alternate Function (I2C)
    ; -------------------------------------------------------------
    LDR R0, =GPIOB_MODER
    LDR R1, [R0]
    BIC R1, R1, #(0xF << 16)    
    ORR R1, R1, #(0xA << 16)    ; AF Mode (10) for PB8, PB9
    STR R1, [R0]

    LDR R0, =GPIOB_OTYPER
    LDR R1, [R0]
    ORR R1, R1, #(0x3 << 8)     ; Open-Drain for PB8, PB9
    STR R1, [R0]

    LDR R0, =GPIOB_PUPDR
    LDR R1, [R0]
    BIC R1, R1, #(0xF << 16)    
    ORR R1, R1, #(0x5 << 16)    ; Pull-ups for PB8, PB9
    STR R1, [R0]

    LDR R0, =GPIOB_AFRH
    LDR R1, [R0]
    BIC R1, R1, #(0xFF << 0)    
    ORR R1, R1, #(0x44 << 0)    ; AF4 (I2C1) for PB8, PB9
    STR R1, [R0]

    ; -------------------------------------------------------------
    ; 3. Initialize I2C1 (Standard Mode 100kHz)
    ; -------------------------------------------------------------
    LDR R0, =I2C1_CR2
    MOV R1, #16                 ; 16MHz APB1 clock
    STR R1, [R0]

    LDR R0, =I2C1_CCR
    MOV R1, #80                 ; 16MHz / (2 * 100kHz)
    STR R1, [R0]

    LDR R0, =I2C1_TRISE
    MOV R1, #17                 ; Max rise time + 1
    STR R1, [R0]

    LDR R0, =I2C1_CR1
    LDR R1, [R0]
    ORR R1, R1, #(1 << 0)       ; Enable PE bit
    STR R1, [R0]
...
init_loop
    ...
    ; -------------------------------------------------------------
    ; 5. Clear the Screen (Fill GRAM with 0x00)
    ; -------------------------------------------------------------
    BL OLED_Clear

    ; -------------------------------------------------------------
    ; 6. Print Row 1: <Programming My>
    ; -------------------------------------------------------------
    MOV R1, #0                  ; Page 0 (Top Row)
    MOV R2, #16                 ; Column 16 (Indent slightly)
    BL OLED_SetCursor
    LDR R4, =String1
    BL OLED_PrintString

    ; -------------------------------------------------------------
    ; 7. Print Row 2: <<STM32F4XX Board>>
    ; -------------------------------------------------------------
    MOV R1, #2                  ; Page 2 (Bottom Row, skips Page 1 to prevent overlap)
    MOV R2, #8                  ; Column 8
    BL OLED_SetCursor
    LDR R4, =String2
    BL OLED_PrintString
...
hang
    B hang                      ; Infinite loop to keep display on
    ENDP
...
String1 DCB "<Programming My>", 0
String2 DCB "<<STM32F4XX Board>>", 0
    ALIGN
